...
class HNFormerConv(nn.Module):
    '''
    one layer of NodeFormer that attentive aggregates all nodes over a latent graph
    return: node embeddings for next layer, edge and subgraph losses at this layer
    '''
    ...
    def __init__(self, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation,
                 ...
                 use_edge_loss=True, use_subgraph_loss=True):
        super(HNFormerConv, self).__init__()
        ...
        self.use_subgraph_loss = use_subgraph_loss
    ...
    def forward(self, z, adjs, subs_dict, tau):
        ...
        # Consider higher-order adjacency as relational bias for better expressiveness at some expense of efficiency.
        ...
        sub_loss = 0
        if self.use_subgraph_loss: # compute subgraph regularization loss
            emb = z_next.squeeze(0)
            for idx in subs_dict.keys():
                source_emb = emb[idx].unsqueeze(1)
                sub_embs_mean = []
                sub_embs_mean_neg = []
                for sub in subs_dict[idx]:
                    sub_embs_mean.append(torch.mean(emb[sub[0]], dim=0).tolist())
                    sub_embs_mean_neg.append(torch.mean(emb[sub[1]], dim=0).tolist())

                sub_embs_mean = torch.tensor(sub_embs_mean).T
                sub_embs_mean_neg = torch.tensor(sub_embs_mean_neg).T

                sub_embs_mean = sub_embs_mean.to(source_emb.device)
                sub_embs_mean_neg = sub_embs_mean_neg.to(source_emb.device)

                source_emb_clone = source_emb.clone()
                score_pos = torch.exp(torch.cosine_similarity(source_emb_clone, sub_embs_mean, dim=0))
                score_neg = torch.exp(torch.cosine_similarity(source_emb_clone, sub_embs_mean_neg, dim=0))

                score = torch.log(score_pos / (score_pos + score_neg))
                sub_loss += torch.sum(score)

            sub_loss = sub_loss / len(subs_dict.keys())

        return z_next, link_loss, sub_loss

class HNFormer(nn.Module):
    '''
    HNFormer model implementation
    return: embedding, predicted node labels, a list of edge and subgraph losses at every layer
    '''

    def __init__(self, init_channels, input_channels, hidden_channels, out_channels, num_layers=2, num_heads=4,
                 dropout=0.0,
                 kernel_transformation=softmax_kernel_transformation, nb_random_features=30, use_bn=True,
                 use_gumbel=True,
                 use_residual=True, use_act=True, use_jk=False, nb_gumbel_sample=10, rb_order=0, rb_trans='sigmoid',
                 use_edge_loss=True, use_subgraph_loss=True):
        super(HNFormer, self).__init__()

        self.convs = nn.ModuleList()
        self.fcs = nn.ModuleList()
        self.bns = nn.ModuleList()
        for d in init_channels:
            self.fcs.append(nn.Linear(d, input_channels))
            self.bns.append(nn.LayerNorm(input_channels))
        self.fcs.append(nn.Linear(input_channels, hidden_channels))
        self.bns.append(nn.LayerNorm(hidden_channels))
        for i in range(num_layers):
            self.convs.append(
                HNFormerConv(hidden_channels, hidden_channels, num_heads=num_heads,
                               ...
                               use_subgraph_loss=use_subgraph_loss))
            self.bns.append(nn.LayerNorm(hidden_channels))
    ...
    def forward(self, x_list, adjs, subs_dict, tau=1.0):
        z_list = []
        for i in range(len(x_list)):
            ...
            z_list.append(z)
        x = torch.cat(z_list, dim=0)
        x = x.unsqueeze(0) # [B, N, H, D], B=1 denotes number of graph
        layer_ = []
        link_loss_ = []
        sub_loss_ = []
        z = self.fcs[len(x_list)](x)
        if self.use_bn:
            z = self.bns[len(x_list)](z)
        z = self.activation(z)
        z = F.dropout(z, p=self.dropout, training=self.training)
        layer_.append(z)

        for i, conv in enumerate(self.convs):
            z, link_loss, sub_loss = conv(z, adjs, subs_dict, tau)
            link_loss_.append(link_loss)
            sub_loss_.append(sub_loss)
            if self.use_residual:
                z += layer_[i]
            if self.use_bn:
                z = self.bns[len(x_list) + i + 1](z)
            if self.use_act:
                z = self.activation(z)
            z = F.dropout(z, p=self.dropout, training=self.training)
            layer_.append(z)
        ...
        return emb, x_out, link_loss_, sub_loss_
